    ...
    override fun parse(input: InputStream): Sequence<ParsedChannel> = sequence {
        ...
        input.bufferedReader().useLines { lines ->
            ...
            lines.forEach { rawLine ->
                ...
                // Reject content that is clearly not an M3U playlist (e.g. an
                // HTML page reached by mistake — common cause: the user pasted
                // a github.com/.../blob/... URL instead of the raw one). Before
                // this check the parser would yield every HTML line as a fake
                // "channel" entry.
                if (!headerSeen) {
                    if (line.startsWith(M3U_HEADER_MARK, ignoreCase = true)) {
                        headerSeen = true
                        // Header consumed — do not also yield it as content.
                        return@forEach
                    }
                    if (line[0] != '#') {
                        throw IllegalArgumentException(
                            "La URL no devuelve una lista M3U válida " +
                                    "(falta cabecera #EXTM3U). " +
                                    "Si es un enlace de GitHub, usa la URL «Raw»."
                        )
                    }
                    // Some uploaders omit the header but start with a valid
                    // M3U tag (e.g. #EXTINF). Accept the file as M3U from
                    // there and fall through into the normal tag branch.
                    headerSeen = true
                }
